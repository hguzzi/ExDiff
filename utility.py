...
def create_graph(model, parameters):
  if model=='erdos-reny':
      ...
      G = xx.erdos_renyi_graph(n, p)
  elif model == 'GNP':  # Added GNP model
      n = parameters['n']  # Number of nodes
      p = parameters['p']  # Probability of edge creation
      G = xx.gnp_random_graph(n, p)  # Use gnp_random_graph
  elif model == 'GNP':  # Added GNP model
      n = parameters['n']  # Number of nodes
      p = parameters['p']  # Probability of edge creation
      G = xx.gnp_random_graph(n, p)  # Use gnp_random_graph
      ...
      G = xx.barabasi_albert_graph(n, m)  # Use barabasi_albert_graph
        ...
        G = xx.stochastic_block_model(sizes, probs, seed=0)  # Use stochastic_block_model
  return G
